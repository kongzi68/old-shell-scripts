...
class ScriptsForm(FlaskForm):
    """ 定义配置管理表单 """
    custom_name = SelectField(label=u"请选择脚本", id="id_custom_name", coerce=int,
                              render_kw={"class": "form-control"})
    pkg_name = StringField(label=u'更新包的名称', validators=[DataRequired(u"请输入更新包的名称")],
                           description=u"请输入更新包的名称",id="id_pkg_name",
                           render_kw={"required": "required",
                                      "class": "form-control",
                                      "placeholder": u"更新包的名称（文件名）"})
    pkg_md5 = StringField(label=u'更新包的MD5', validators=[DataRequired(u"请输入更新包的MD5")],
                          description=u"请输入更新包的MD5",id="id_pkg_md5",
                          render_kw={"required": "required",
                                     "class": "form-control",
                                     "placeholder": u"更新包的MD5码"})
    update_target = SelectField(label=u'需更新的平台', id="id_update_target",
                                choices=[('ALL', 'ALL'), ('IOS', 'IOS'), ('AOS', 'AOS')],
                                render_kw={"class": "form-control"})
    msgsid = HiddenField(label='msgsid', id='id_set_msgsid')
    submit = SubmitField(label=u'执行脚本')
